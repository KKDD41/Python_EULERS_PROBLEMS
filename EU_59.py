...
def check_english(ascii1, ascii2):
    ...
    """
    xor = ascii1 ^ ascii2 # вычитает \\ складывает, короче однозначно как-то связывает, разберись
    if 32 <= xor <= 90:
        return True
    elif 97 <= xor <= 122:
        return True
...
f = open('cipher.txt')
cipher = f.read().strip().split(',') # читает как str
cipher = [int(x) for x in cipher]

# генератор строчных букв ascii
